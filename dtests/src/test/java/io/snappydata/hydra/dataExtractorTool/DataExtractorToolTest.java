...
public class DataExtractorToolTest extends SnappyTest {
  ...
  private static final long serialVersionUID = 7085700669843656387L;
  ...
  public void executeQuery() {
    ...
    Connection conn;
    ArrayList<String> queryArray = getQueryArr(queryFile);
    try {
      conn = getLocatorConnection();
      for (String queryStr : queryArray) {
        try {
  ...
  public void validate(Integer expectedCnt, Integer unExpectedCnt) {
    if (!unExpectedExceptionCnt.equals(unExpectedCnt))
    ...
    if (!expectedExceptionCnt.equals(expectedCnt))
  ...
  public void createDummyData() {
    ...
    try {
      ...
      Log.getLogWriter().info(
          "Caught exception as the servers are down with OOME as isOOME = " + isOOME);
    }
  ...
  public ArrayList<String> getQueryArr(String fileName) {
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        for (String splitDatum : splitData) {
          if (splitDatum != null && splitDatum.length() > 0) {
            queries.add(splitDatum);
          }
        }
      }
      br.close();
