...
class CommandLineToolsSuite extends SnappyTestRunner {
  ...
  test("exec scala") {
    ...
    try {
      ...
      assert(stmnt.execute("exec scala snappysession.sql(" +
          "\"insert into test_app values(10, 10), (20, 20)\")"))
      var rs = stmnt.getResultSet
      assert(rs.next())
      assert(stmnt.execute("exec scala snappysession.sql(" +
          "\"create table test_exec(col1 int not null, col2 int not null)\")"))
      ...
      assert(stmnt.execute("exec scala val df1 = snappysession.table(\"test_app\")\n" +
          ...
          "df3.show"))
      stmnt.execute("exec scala options(returnDF 'df33') " +
          "val df33 = snappysession.table(\"test_exec\")")
      rs = stmnt.getResultSet
      assert(rs.next())
      assert(rs.getMetaData.getColumnCount == 2)
      assert(stmnt.execute("exec scala options(returnDF 'df4') " +
          "val df4 = snappysession.sql(\"select * from test_exec\")"))
      rs = stmnt.getResultSet
      assert(rs.next())
      assert(rs.getMetaData.getColumnCount == 2)
      ...
      rs = stmnt.getResultSet
      assert(!rs.next())
      assert(stmnt.execute("exec scala options(returnDF 'ds2') " +
  ...
  ignore("ODBC_FailOverTest_NEWSERVER") {
    try {
      val scriptPath = s"$snappyNativeTestDir/failoverTest_NewServer.sh"
      val consoleOutput = s"$scriptPath $snappyProductDir $snappyNativeTestDir".!!
  ...
  ignore("ODBC_FailOverTest_NONE") {
    try {
      val scriptPath = s"$snappyNativeTestDir/failoverTest_None.sh"
      val consoleOutput = s"$scriptPath $snappyProductDir $snappyNativeTestDir".!!
