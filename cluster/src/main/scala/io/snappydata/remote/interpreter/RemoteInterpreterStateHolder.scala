...
class RemoteInterpreterStateHolder(
  ...
  private val sc: SparkContext = SparkContext.getOrCreate()
  private val snappy = SnappySessionPerConnection.getSnappySessionForConnection(connId)

  snappy.conf.set(Attribute.USERNAME_ATTR, user)
  snappy.conf.set(Attribute.PASSWORD_ATTR, authToken)

  private lazy val pw = new StringPrintWriter()
  private lazy val strOpStream = new StringOutputStream(pw)
  ...
  private val allInterpretedLinesForReplay: mutable.ArrayBuffer[String] =
  ...
  private def initIntp(): Unit = {
    ...
    intp.interpret("@transient val sc = snappysession.sparkContext\n")
    pw.reset()
  }
  ...
  def interpret(code: Array[String], options: Map[String, String]): AnyRef = {
    interpret(code, replay = false, options)
  }
  ...
  def interpret(code: Array[String], replay: Boolean,
    ...
    try {
      logDebug(s"Interpreting lines ${code.mkString(" ; ")} for user $user on conn $connId")
  ...
  def processCommand(command: String): Array[String] = {
    ...
    commandMethod().invoke(this.intp, command)
  ...
  private lazy val modifiedHelpHeaderLine = (s: String) => {
    ...
    val nonModifiableStmnt = "\nSome Commands that cannot be abbreviated are: 'run', " +
        "'elapsedtime on',\n  'maximumdisplaywidth' and 'maximumlinewidth'\n"
  ...
  private def getSettings(replOpPath: String): Settings = {
    ...
    gs.processArguments(interpArguments, processAll = true)
    gs
  }

  private def deleteAndCreateDir(): Unit = {
    ...
    try {
      ...
      case _: IOException if !reachedCreation => // ignore. may be it does not exists anymore
    }
  }
  ...
  def createSparkILoop: SparkILoop = {
    ...
    // s" to lead loader = ${lead.urlclassloader} with obj " +
    // System.identityHashCode(lead.urlclassloader))
  ...
  def close(): Unit = {
    ...
    strOpStream.close()
    pw.close()
    ServiceManager.getLeadInstance.asInstanceOf[LeadImpl]
  ...
  def replayCmd(): Unit = {
    if (allInterpretedLinesForReplay.nonEmpty) {
      ...
      resetCmd()
      interpret(copy.toArray, replay = true)
    } else {
  ...
  class StringOutputStream(val spw: StringPrintWriter) extends OutputStream {
...
object RemoteInterpreterStateHolder {
  ...
  private val replOutputDir = sc.getConf.get("spark.local.dir", "/tmp")
}
...
class RemoteILoop(spw: StringPrintWriter, intpHelper: RemoteInterpreterStateHolder)
  ...
  lazy val serviceableCommands: List[RemoteILoop.this.LoopCommand] = {
    val inheritedCommands = super.commands.filterNot(
