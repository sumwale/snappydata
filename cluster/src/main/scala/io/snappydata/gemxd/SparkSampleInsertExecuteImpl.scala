...
import org.apache.spark.sql.{DataFrame, Row, SparkSupport}
import org.apache.spark.util.SnappyUtils

/**
 * Encapsulates a Spark insert operation for use in query routing from JDBC
 * for insertion into sample tables for a base table if there are any.
 */
class SparkSampleInsertExecuteImpl(val baseTable: String,
    ...
    extends SparkSQLExecute with SparkSupport with Logging {
  ...
  override def packRows(msg: LeadNodeExecutorMsg,
    ...
    val rows = if (this.dvdRows != null) {
      dvdRows.asScala.map(dvdArr => Row.fromSeq(dvdArr.map(
        org.apache.spark.sql.SnappySession.getValue(_, returnUTF8String = false))))
    } else {
...
object SparkSampleInsertExecuteImpl {
  def insertIntoSampletables(baseTable: TableIdentifier, ds: DataFrame,
    ...
    aqpRelations.foreach {
      case (lr: LogicalRelation, _) if lr.relation.isInstanceOf[SamplingRelation] =>
        lr.relation.asInstanceOf[SamplingRelation].insert(ds, overwrite = false)
    }
  }
}
