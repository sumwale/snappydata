...
class SparkSQLExecuteImpl(
  ...
  session.snappySessionState.setJdbcQueryJobGroupId(ctx.getStatementId.toString)
  ...
  override def packRows(msg: LeadNodeExecutorMsg,
    ...
    val rddId = df match {
      ...
      case _ => -1 // not used for this case
    }
    ...
    try {
      ...
      val partitionBlocks = df match {
        ...
        case _ =>
          Iterator(CachedDataFrame(null,
            df.queryExecution.executedPlan.executeCollect().iterator)._1)
      }
      hdos.clearForReuse()
      ...
      if (!blockReadSuccess && df.isInstanceOf[CachedDataFrame]) {
...
object SnappySessionPerConnection {
  ...
  def getSnappySessionForConnection(connId: java.lang.Long): SnappySession = {
    val session = connectionIdMap.computeIfAbsent(connId, CreateNewSession)
    if (session.sparkContext.isStopped) {
      connectionIdMap.remove(connId)
      connectionIdMap.computeIfAbsent(connId, CreateNewSession)
    } else {
      session
    }
  }
