...
object SnappyMetricsSystem {
  ...
  private val oldSizeMap = collection.mutable.Map.empty[String, Int]
  private val MAX_SIZE = 180

  @volatile private var doRun: Boolean = false
  @volatile private var running: Boolean = false

  private var tables = ArrayBuffer.empty[String]
  private var externalTables = ArrayBuffer.empty[String]

  private def logger: LogWriterI18n = Misc.getGemFireCache.getLoggerI18n

  def init(sc: SparkContext): Unit = {
    GemFireXDUtils.waitForNodeInitialization()
    // initialize metric system with cluster id as metrics namespace
    val clusterUUID = Misc.getMemStore.getMetadataCmdRgn.get(CLUSTER_ID).asInstanceOf[String]
  ...
  private def putMembersDiskStoreIdInRegion(): Unit = {
    try {
  ...
  def convertStatsToMetrics(): Unit = synchronized {
    try {
      val cache = Misc.getGemFireCacheNoThrow
      if (doRun && cache != null) {
        running = true
  ...
  private def createHistogram(metricName: String, metricValue: Long): Unit = {
  ...
  private def setMetricsForClusterStatDetails(): Unit = {
  ...
  private def removeTableMetricsIfTableDeleted(
      stringToStats: Map[String, SnappyRegionStats]): Unit = {
    for (t <- tables) {
  ...
  private def setMetricsForTableStatDetails(tableBuff: Map[String, SnappyRegionStats]): Unit = {
  ...
  private def removeExternalTableMetricsIfTableDeleted(
      stringToStats: Map[String, SnappyExternalTableStats]): Unit = {
  ...
  private def setMetricsForExternalTableStatDetails(
      externalTableBuff: Map[String, SnappyExternalTableStats]): Unit = {
  ...
  private def setMetricsForMemberStatDetails(membersBuff: mutable.Map[String, MemberStatistics]) {
