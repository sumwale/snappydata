  ...
  def convertStatsToMetrics(member: String, memberDetails: MemberStatistics) {
    ...
    try {
      ...
      if (cache != null) {
        ...
        createGauge(s"$namespace.diskStoreUUID",
          memberDetails.getDiskStoreUUID.asInstanceOf[AnyVal])
        createGauge(s"$namespace.diskStoreName",
          memberDetails.getDiskStoreName.asInstanceOf[AnyVal])
        ...
        createGauge(s"$namespace.offHeapExecutionPoolUsed",
          memberDetails.getOffHeapExecutionPoolUsed)
        createGauge(s"$namespace.offHeapExecutionPoolSize",
          memberDetails.getOffHeapExecutionPoolSize)
